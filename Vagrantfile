...
# this is the directory where the xcore and xcore-extensions
# cloned code repositories are located
sourceDir = "../source"

Vagrant.configure("2") do |config|

	# We can assign this box a name so it won't conflict with
	# other boxes running on this machine. i.e 'default'
	#config.vm.define "boxname" do |fb|
	#end
  ...
  config.vm.network :forwarded_port, guest: 80, host: 8887, auto_correct: true 
  config.vm.network :forwarded_port, guest: 8443, host: 8447, auto_correct: true
  ...
  config.vm.synced_folder sourceDir, "/home/vagrant/source"
